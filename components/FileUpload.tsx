...
const FileUpload: React.FC<FileUploadProps> = ({
  ...
  const handleBSIChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      // Using .includes() avoids rejection if the OS adds (1) or changes case "Account Statement (1).csv"
      if (!fileNameUpper.includes("ACCOUNT STATEMENT") || !fileNameUpper.includes("2477946710")) {
  ...
  const handleMuamalatChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (!fileNameUpper.includes("ACC-STATEMENT")) {
