...
const ResultTable: React.FC<ResultTableProps> = ({ data, onReset, selectedDate, selectedPaymentMethod }) => {
  ...
  const processedData = useMemo(() => {
    ...
    // Safely sort considering optional/missing variables to fix TypeScript errors
    result.sort((a, b) => {
      let comparison = 0;

      switch (sortConfig.key) {
        case 'nominal':
          comparison = (a.nominal || 0) - (b.nominal || 0);
          break;
        case 'tanggal':
          const timeA = (a as any).rawDate ?? new Date((a.tanggal || '').split('/').reverse().join('-')).getTime();
          const timeB = (b as any).rawDate ?? new Date((b.tanggal || '').split('/').reverse().join('-')).getTime();
  ...
  const totalNominal = useMemo(() => {
    return processedData.reduce((acc, curr) => acc + (curr.nominal || 0), 0);
  }, [processedData]);
  ...
  const handleCopyCustomFormat = async () => {
    ...
    try {
      // Prevents 'ClipboardItem is undefined' TS compiler issues
            ...
            <tbody className="divide-y divide-gray-100 text-sm">
              {processedData.length > 0 ? (
                processedData.map((row, index) => (
                  <tr key={row.id || `res-row-${index}`} className="hover:bg-blue-50/50 transition-colors">
